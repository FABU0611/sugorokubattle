...
HRESULT InitRenderer(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	//フォントの作成
	g_Font = CreateFont(48, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
...
void DebugTextOut(char* text, int x, int y)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			//システムのフォントを保存する
			g_OriginalFont = (HFONT)SelectObject(hdc, g_Font);
			...
			//フォントを元に戻す
			SelectObject(hdc, g_OriginalFont);
