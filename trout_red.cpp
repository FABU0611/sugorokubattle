...
void TROUT_RED::Init(void)
{
	...
	{//頂点バッファの中身を埋める
		...
		// UV値の設定
		pVtx[0].TexCoord = D3DXVECTOR2((1.0f / 6) * 0, 0.0f);
		pVtx[1].TexCoord = D3DXVECTOR2((1.0f / 6) * 0, 1.0f);
		pVtx[2].TexCoord = D3DXVECTOR2((1.0f / 6) * 1, 0.0f);
		pVtx[3].TexCoord = D3DXVECTOR2((1.0f / 6) * 1, 1.0f);
...
void TROUT_RED::Event(){
	...
	//敵を出現させる
	em.Add(new ENEMY({ 0.0f, 0.0f, 300.0f }, true));
	em.Add(new ENEMY({ 0.0f, 0.0f, -300.0f }, true));
	em.Add(new ENEMY({ 300.0f, 0.0f, 0.0f }, true));
	em.InitEnemyManager();
}
